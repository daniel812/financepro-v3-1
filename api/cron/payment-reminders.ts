...
//
// Standalone by design (own Supabase client, own Telegram sender), mirroring
// api/telegram/webhook.ts: this runs server-side with no browser session, so it
// needs the service-role key rather than the anon-key client used by lib/db.ts.
...
export default async function handler(req: any, res: any) {
  ...
  if (!BOT_TOKEN) {
    return res.status(200).json({ ok: true, skipped: 'TELEGRAM_BOT_TOKEN no configurado' });
  }
